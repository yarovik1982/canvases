...
export class Plotter {
	...
	constructor(
		ctx,
		canvases = [],
		{ width, height, x0, y0, xMin, xMax, yMin, yMax },
	) {
		// Свойства:
		this.ctx = ctx;
		...
		this.x0 = options.x0;
		this.y0 = options.y0;
		this.xScale = (options.xMax - options.xMin) / options.width;
		this.yScale = (options.yMax - options.yMin) / options.height;
		this.xMin = options.xMin / this.xScale;
		this.xMax = options.xMax / this.xScale;
		this.yMin = options.yMin / this.yScale;
		this.yMax = options.yMax / this.yScale;
	}
	drawGrid(xCell, yCell, color = "lightgray") {
		if (this.canvases.length > 0) {
			for (let i = 0; i < this.canvases.length; i++) {
				...
				xCell /= this.xScale;
				yCell /= this.yScale;
				for (let x = this.xMin; x <= this.xMax; x += xCell) {
					for (let y = this.yMin; y <= this.yMax; y += yCell) {
